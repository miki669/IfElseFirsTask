...
public static class UserExtenstion
    ...
    public class TokenAuthHandler : AuthenticationHandler<TokenAuthOptions>
    {
        ...
        private async Task<AuthenticateResult> ValidateToken(string token)
        {
            ...
            using var context = scope.ServiceProvider.GetService<PrimaryDataBaseContext>();
            ...
            await context.Entry(validatedToken).Reference(p => p.Account).LoadAsync();
            await context.Entry(validatedToken.Account).Collection(p => p.Roles).LoadAsync();
            string[] roles = validatedToken.Account.Roles.Select(p => p.Name.ToLower()).ToArray();
